...
\begin{figure}
    ...
    \caption{Samples from a 2D 2-Gaussian mixture using Metroplois Hastings: $N=10,000$ samples.}
    \label{fig:MH_GMM}
\end{figure}

Fig.~\ref{fig:MH_GMM} shows an implementation of Metropolis Hastings to sample from a mixture of 2 Gaussians using a single Gaussian as proposal.
...
Fig.~\ref{fig:MH_GMM} shows an implementation of Gibbs and  Metropolis Hastings to sample from a correlated Gaussian. Observe how with 1,000 samples Gibbs fully explores the support of the distribution while MH struggle to propose valid sample.
...
\begin{remark}
    The coordinate-wise densities have not been fixed to an explicit expression. Such expressions can be chosen for each coordinate based on its features, e.g., whether they are continuous or discrete. 
\end{remark}
...
\begin{example}
    We will see that the MF family captures marginals but no dependencies between variables (in particular, correlations). Let us consider $z = [z_1,z_2]^\top\in\R^2$, and the target 
    \[
    p(z \mid x)
    =
    \cN\!\left(
    ...
    \right).
    \]
    The optimal MF family for this target is\footnote{We will not prove this, as it requires results from variational calculus.}  $q(z) = q_1(z_1)q2(z_2)$. Denoting 
    \[
    q_1(z_1) = \cN(z_1 \mid m_1, s_1), \quad  q_2(z_2) = \cN(z_2 \mid m_2, s_2), 
    \]
    let us find the optimal variational parameters $\theta = \{m_1,s_1,m_2,s_2\}$. That is
    \[
    \theta^* = \argmin \KL{q_1(z_1)q_2(z_2)}{p(z_1,z_2 \mid x)}
    \]

    Since both \(q\) and \(p\) are Gaussian, the KL divergence admits a closed-form
    expression. Writing \(q(z)=\mathcal N(z\mid m, S)\) with
    \[
    m =
    \begin{bmatrix}
    m_1\\
    ...
    \Big),
    \end{align*}
    where \(\mu=(\mu_1,\mu_2)^\top\) and \(\Sigma=(\sigma_{ij})_{i,j=1}^2\).
    ...
    \]
    which vanishes if and only if \(m_1=\mu_1\). By symmetry, \(m_2=\mu_2\).

    Next, differentiating with respect to \(s_1\) gives
    \[
    ...
    To conclude, let us explore what the recovered variance means. Since we can calculate the precision matrix explicitly for this case, we have 
    \[
    s_1 = \frac{1}{(\Sigma^{-1})_{11}} = \frac{\sigma_{22}}{\sigma_{11}\sigma_{22} - \sigma_{12^2}} = \sigma_{11}(1-\rho^2),
    \]
    where $\rho = \sigma_{12}/\sqrt{\sigma_{11}\sigma_{22}}$ is the correlation coefficient. The interpretation is as follows. 
    ...
    \item When \(|\rho| > 0\), the mean-field variance is strictly smaller than the
    marginal variance, reflecting the fact that dependencies between variables are
    ignored by the mean-field assumption.
    \item As \(|\rho| \to 1\), we have \(s_1 \to 0\), indicating that strong correlation
    makes \(z_1\) almost deterministically determined by \(z_2\), a phenomenon that
    cannot be captured by a mean-field approximation.
    \end{itemize}

    More generally, this illustrates that mean-field variational inference
    systematically underestimates posterior uncertainty in the presence of strong
    correlations. Fig.\ref{fig:MF_Gaussian} shows this approximation for three different cases. 
