...
We equip $\cX,\cY,\cZ$ with their Borel $\sigma$-algebras $\cB(\cX),\cB(\cY),\cB(\cZ)$, and consider the product measurable space
\[
(\cX\times\cY\times\cZ,\; \cB(\cX)\otimes\cB(\cY)\otimes\cB(\cZ)).
\]
Then the joint random variable $(X,Y,Z):\Omega\to \cX\times\cY\times\cZ$ is measurable with respect to $\cF$ and the above product $\sigma$-algebra. 

Furthermore, we will assume that the law of $(X,Y,Z)$ is absolutely continuous with respect to the product base measure on $\cX\times\cY\times\cZ$ (e.g.\ Lebesgue measure when $\cX,\cY,\cZ\subseteq\R^d$), and hence admits a joint density $p(x,y,z)$. That is, for all $A\in\cB(\cX),B\in\cB(\cY),C\in\cB(\cZ)$,
\begin{equation}
...
\begin{example}[Generative and discriminative views of binary classification]
	...
	\begin{align}
		p(Y=1 | X=x) 	&= \frac{p(X=x|Y=1)p(Y=1)}{p(X=x)}\nonumber \\
						&= \frac{1}{1 + e^{-\log\left(\frac{\pi}{1-\pi}\frac{f_{\theta_1}(x)}{f_{\theta_0}(x)}\right)}}.\label{eq:generative_binary_classification}
	\end{align}
\end{example}
...
\begin{remark}
	...
	\begin{equation}
	(T_\#P_X)(A) \;=\; P_X(T^{-1}(A)), \qquad \forall A\in\mathcal B(\mathcal X).
	\end{equation}
...
\begin{remark}
	...
	\begin{equation}
		l(\theta) = \log L(\theta) = \sum_{n=1}^N \log p_\theta(y_n).
	\end{equation}
\end{remark}
...
\begin{example}[Gaussian linear regression]
	...
	\end{equation}
	This is equivalent to $Y=ax + \epsilon, \epsilon\sim\cN(0,\sigma^2)$. The parameters in this setting are  $\theta = (a,\sigma^2)$. Now consider the observations $\{(x_n,y_n)\}_{n=1}^N$. 

	Since $p(y_n|x_n) = \frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(\frac{-1}{2\sigma^2}(y_n-ax_n)^2\right)$, we can write the log-likelihood as 
	\begin{equation}
		l(\theta) = \sum_{n=1}^{N} \frac{-1}{2}\log 2\pi\sigma^2 - \frac{1}{2\sigma^2}(y_n-ax_n)^2. \label{eq:ML_linear_regression}
	\end{equation}
...
\begin{example}[Binary classification]
	Consider observations $\{(x_n,y_n)\}_{n=1}^N\subset \R^d\times \{0,1\}$ from a binary classification setting. Model the classifier as 
	\begin{equation}
		p_\theta(y=1|x) = \sigma(s(x)),
	\end{equation}
	where $\sigma(s(x)) = \frac{1}{1+e^{-s(x)}}$, and $s:\R^d\to \R$ is a feature extractor (e.g., $s(x) = a^\top x + b$). Assuming that the observations are i.i.d., we have
	\begin{equation}
		L(\theta) = \prod_{n=1}^N p(y_n|x_n) = \prod_{n=1}^N \sigma(s(x_n))^{y_n} (1-\sigma(s(x_n)))^{1-y_n}, 
	\end{equation}
	and equivalently
	\begin{equation}
		l(\theta) = \sum_{n=1}^N y_n \log \sigma(s(x_n)) + (1-y_n)\log (1-\sigma(s(x_n))). 
	\end{equation}
	Does this expression seem familiar? If not, we will find out soon what this is.
\end{example}

\begin{example}[Clustering]
Consider a set of observations $\{x_n\}_{n=1}^N\in\R^d$ and implement a clustering algorithm. We will assume that there are $K\in\N$ clusters, each specified by a density $p_k, k=1,\ldots, K$; this means that the probability of the RV $X$ coming from the $k$-th cluster is $\P(X\in C_k) = \pi_k$, where $\forall k, 0\leq \pi_k \leq 1$ and $\sum_{k=1}^K \pi_k = 1$. 
...
\begin{equation}
	l(\theta) = \sum_{n=1}^N \log \sum_{k=1}^K \pi_k p_k(x_n) \label{eq:clustering_likelihood}
\end{equation}
...
\begin{itemize}
	...
	\item The problem is ill-posed. E.g., if $p_k=\cN(\mu_k,\Sigma_k)$, which is the usual choice, we can set $\mu_k=x_n, \Sigma_k=0$ which gives $l=\infty$.
\end{itemize}	
...
\textbf{NB:} This section is based on Chapter 6 of \cite{murphy2022pml_intro}
...
\begin{equation}
	H_{CE}(p_X,q_X) = -\sum_{x=1}^K p_X(x) \log q_X(x).
\end{equation}
...
\section[KL divergence as a way to compare distributions]{KL divergence as a metric to compare $p$ and $q$}
...
\begin{example}[KL versus $L_1$]
	Consider $p(x)=\uni{0,1}$ and $q_n(x) = \one{x\in[0,1/n]}e^{-n} + \one{x\in[1/n,1]} c_n $, with $c_n\ge 0$ so that $q$ integrates 1 ($n > 1$). 
	
	Let us first compute $c_n$ explicitly. Since $q_n$ must integrate to one, we require $\int_0^{1/n} e^{-n}\,\mathrm{d}x + \int_{1/n}^1 c_n\,\mathrm{d}x = 1$,
	which yields
	\[
	...
	\begin{equation}
		c_n = \frac{1-\frac{1}{n}e^{-n}}{1-\frac{1}{n}} = \frac{n-e^{-n}}{n-1}.	\label{eq:cn_KL_example}
	\end{equation}
	...
	\end{align}
	Now take $n\to\infty$. From eq.~\eqref{eq:cn_KL_example} we can see that $c_n$ converges to 1. Therefore, $\DL{p}{q}\to 0$. However, note that $\KL{p}{q} \to 1$.  
\end{example}
...
\begin{example}[Assymetry of the KL between two Gaussians]
	...
	\end{itemize}
	Fig.~\ref{fig:asymmetric_KL} shows these functions, note how the penalisation strength depends on the direction.
\begin{figure}[H]
	\centering
...
\begin{example}[KL gradient flow]
	...
	\end{align}
	Figure \ref{fig:asymmetric_KL_flow_Gaussian} implements these recursions. 
	
	\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{img/week1_KL_gradient_flow.pdf}
	\caption{KL gradient flow between two Gaussians.}
	\label{fig:asymmetric_KL_flow_Gaussian}
\end{figure}
	
\end{example}

\paragraph{KL and maximum likelihood.} Let us now return to the setting of the learning problem. Consider a true model given by $p$ and iid observations $x_1,x_2,\ldots,x_N\sim p(x)$. We could use the KL to look for the best approximator of $p$ with a given family of candidate models $\{q_\theta, \theta\in\Theta\}$. That is, 
\begin{equation}
	\theta^\star = \argmin \KL{p}{q_\theta}.
\end{equation}
...
\begin{align}
		\theta^\star 
		&= \argmin \int_\cX p(x) \log p(x)\dx - \int_\cX p(x) \log q_\theta(x)\dx\\
		&= \argmax \mathbb{E}_p[\log q_\theta(x)]\\
		&\approx \argmax \sum_{x_i}\log q_\theta(x_i),
\end{align}
...
As we will throughout the module, when designing/chossing a PGM, we will be faced with the following scenarios. 

\begin{itemize}
	\item \textbf{Explicit-likelihood models:} These include classical statistical models such as Gaussians, exponentials, Bernoulli, $Xi^2$, log-normal, but also combinations or transformations that we construct as long as they have an explicit likelihood. These are Gaussian mixtures, piece-wise defined distributions, and amy pushforward model constructed through an invertible transformation so that its density can be calculated via the change of variable theorem. These last models are referred to as normalising flows, as they assume a Gaussian source measure. 
	\item \textbf{Implicit models:} As the name suggests, these models are only implicitly defined via a data-generating mechanism, usually involving sampling. For instance, take a RV $Z\in\R^d\sim\cN(0,I_d)$ and construct $X=T_\theta(Z)$, where $T_\theta$ is collection of neural networks which are sequentially applied to $Z$ with the aim to replicate learnt dynamics that make $X$ \emph{flow} towards the desired distribution. Depending on the parametrisation, these models are known as score-based models, diffusion models, or flow matching.   
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Generative vs discriminative modelling (theory).}  
    Let $(X,Y)$ be random variables with joint distribution $p(x,y)$.  
    \begin{enumerate}
        \item Define what is meant by a \emph{generative model} and a \emph{discriminative model}.
        \item Show how a generative model can be used to construct a classifier.
        \item Discuss one advantage and one limitation of generative modelling relative to discriminative modelling.
    \end{enumerate}
    \item \textbf{Information-theoretic objectives  (theory).}  
    Let $p$ be a data-generating distribution and $q_\theta$ a parametric model.
    \begin{enumerate}
        \item Define the entropy $H(p)$, cross-entropy $H(p,q_\theta)$, and KL divergence $\mathrm{KL}(p\|q_\theta)$.
        \item Show that maximising the log-likelihood of data sampled from $p$ is equivalent to minimising $\mathrm{KL}(p\|q_\theta)$.
        \item Explain why minimising $\mathrm{KL}(p\|q_\theta)$ and $\mathrm{KL}(q_\theta\|p)$ lead to qualitatively different approximations.
    \end{enumerate}
    \item \textbf{Maximum likelihood density estimation  (coursework).}  
    You are given samples from a one-dimensional distribution.
    \begin{enumerate}
        \item Fit a Gaussian model by maximum likelihood.
        \item Fit a mixture of Gaussians by maximum likelihood.
        \item Empirically compare the learned models using log-likelihood and visual inspection.
    \end{enumerate}

    \item \textbf{Forward and reverse KL divergence   (coursework).}  
    Consider approximating a multimodal target distribution using a unimodal Gaussian.
    \begin{enumerate}
        \item Numerically minimise $\mathrm{KL}(p\|q)$ and $\mathrm{KL}(q\|p)$.
        \item Visualise the resulting solutions.
        \item Explain the observed behaviour using the geometry of the KL divergence.
    \end{enumerate}
\end{enumerate}